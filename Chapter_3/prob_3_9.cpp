...
double find_median(int salesArray[], const int SIZE)
{
    ...
    while (salesArray[newSize - 1] < -0.5)
    {
        newSize--;
    }

    int newSales[newSize];
    for (int j = 0; j < newSize; j++)
    {
        newSales[j] = salesArray[j];
    }

    if (newSize % 2 != 0)
    {
        index1 = floor(newSize / 2);
        return newSales[index1];
    }
    halfIndex = newSize / 2.;
    index1 = halfIndex;
    index2 = halfIndex - 1;

    return (newSales[index1] + newSales[index2]) / 2.;
}
...
int main()
{
    ...
    int sales_even[NUM_AGENTS][NUM_EVEN] = {
        {1856, 498, 30924, 87478, 328, 2656, 387, 3754, 387587, 2873, -1, -1},
    ...
    int sales_odd[NUM_AGENTS][NUM_ODD] = {
        {1856, 498, 30924, 87478, 328, 2656, 387, 3754, 387587, 2873, -1, -1, 2257},
